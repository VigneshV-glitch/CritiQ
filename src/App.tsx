...
function AppContent() {
  ...
  // Custom user instruction and correction strategy states
  const [customInstruction, setCustomInstruction] = useState<string>('');
  const [correctionStrategy, setCorrectionStrategy] = useState<string>('Balanced');
  ...
  const handleSelectDemo = (reviewId: string) => {
    ...
    if (rev) {
      ...
      setCorrectionStrategy('Balanced');
      setWorkflowState('diagnosing');
    }
  };

  const handleSelectRecentReview = (reportId: string) => {
    const rev = reviews.find(r => r.id === reportId);
    if (rev) {
      setActiveReport(rev);
  ...
  const handleUploadImage = (fileDataUrl: string, fileName: string, userInstruction?: string, strategy?: string) => {
    setDiagnosingFile({ url: fileDataUrl, name: fileName });
    setCustomInstruction(userInstruction || '');
    setCorrectionStrategy(strategy || 'Balanced');
  ...
  return (
    <div className="w-full h-[100dvh] bg-gradient-to-br from-[#02040c] via-[#040c24] to-[#00458e] flex flex-col overflow-hidden font-sans text-slate-200 relative select-none">
        ...
        <AnimatePresence mode="wait">
          {workflowState === 'landing' && (
            ...
            >
              <Landing
                reviews={reviews}
                onSelectRecentReview={handleSelectRecentReview}
          ...
          {workflowState === 'review' && (
            ...
            >
              <ReviewWorkspace
                ...
                onViewReport={() => setWorkflowState('report')}
                onCloseWorkspace={() => {
                  setActiveReport(null);
                  setWorkflowState('landing');
                }}
          ...
          {workflowState === 'report' && (
            ...
            >
              <ReportView
                report={activeReport}
                onBackToWorkspace={() => setWorkflowState('review')}
                onNewInspection={() => {
                  setActiveReport(null);
                  setSelectedDemoId(null);
                  setWorkflowState('landing');
                ...
                onExportReport={() => {
                  if (activeReport) {
                    ...
                    downloadAnchor.setAttribute("download", `critiq-report-${activeReport.name.toLowerCase().replace(/\s+/g, '-')}.json`);
