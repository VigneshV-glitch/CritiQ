...
export abstract class BaseReviewAgent implements IReviewAgent {
  ...
  async run(screenModel: ScreenModel, userInstruction?: string, correctionStrategy?: string): Promise<AgentResponse> {
    ...
    try {
      ...
      if (correctionStrategy === 'Minimal') {
        strategyGuideline = 'Fix high-friction problems while strictly preserving the existing visual direction and brand style. Avoid suggesting major layout reorganizations or visual style deviations.';
      } else if (correctionStrategy === 'Comprehensive') {
        strategyGuideline = 'Provide a deep redesign-oriented audit with thorough structural improvements, visual layout transformations, and premium aesthetics recommendations.';
      } else {
        strategyGuideline = 'Provide balanced, meaningful improvements while preserving general design direction.';
      }
...
${userInstruction ? `USER'S EXPLICIT INSPECTION INSTRUCTION (HIGHEST PRIORITY FOCUS):
"${userInstruction}"` : ''}

CORRECTION STRATEGY ENFORCED:
- Strategy: "${correctionStrategy || 'Balanced'}"
- Guideline: ${strategyGuideline}
...
2. ${userInstruction ? 'PRIORITIZE evaluating the specific parts, sections, or concepts requested by the user instruction above.' : 'Audit standard UI/UX focus areas.'}
3. Always maintain standard accessibility/usability rules (e.g. WCAG contrast, standard sizing, touch clearance, error prevention) as objective, non-overridable constraints.
4. Every issue MUST reference a component's real ID under "affectedComponents" if relevant.
5. For "boundingBox", MUST copy the exact boundingBox of the affected component or container from the ScreenModel, or use a reasonable coordinate mapping within that element.
6. "severity" MUST be one of: "low", "medium", "high", "critical", "info".
7. Do NOT return natural language paragraphs outside the requested JSON format.
`;
