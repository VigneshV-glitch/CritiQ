...
export class ReviewOrchestrator {
  ...
  public async orchestrate(screenModel: ScreenModel, options: OrchestratorOptions = {}): Promise<UnifiedReviewReport> {
    ...
    const correctionStrategy = options.correctionStrategy;

    // 1. Run all review agents in parallel (or using strategy)
    const agentResponses = await strategy.execute(activeAgentsList, screenModel, userInstruction, correctionStrategy);
