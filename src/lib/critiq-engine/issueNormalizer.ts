...
export class IssueNormalizer {
  ...
  public static normalizeAndMerge(rawIssues: any[]): CritiqIssue[] {
    ...
    rawIssues.forEach((raw, idx) => {
      ...
      // Standardize location classification
      const isGlobal = raw.locationType === 'global' || raw.boundingBox === null;
      const locationType = isGlobal ? 'global' : 'element';
      const locationConfidence = raw.locationConfidence !== undefined ? raw.locationConfidence : (isGlobal ? 100 : 85);

      // Bound coordinate limits
      const x = isGlobal ? 0 : Math.max(0, Math.min(100, typeof raw.boundingBox?.x === 'number' ? raw.boundingBox.x : 10));
      const y = isGlobal ? 0 : Math.max(0, Math.min(100, typeof raw.boundingBox?.y === 'number' ? raw.boundingBox.y : 10));
      const width = isGlobal ? 100 : Math.max(1, Math.min(100 - x, typeof raw.boundingBox?.width === 'number' ? raw.boundingBox.width : 40));
      const height = isGlobal ? 100 : Math.max(1, Math.min(100 - y, typeof raw.boundingBox?.height === 'number' ? raw.boundingBox.height : 10));
      ...
      const issue: CritiqIssue = {
        ...
        boundingBox: isGlobal ? null : { x, y, width, height },
  ...
  private static shouldMerge(a: CritiqIssue, b: CritiqIssue): boolean {
    // Prevent spatial merging if one or both issues represent a global screen-wide concern
    if (a.locationType === 'global' || b.locationType === 'global') {
      if (a.locationType !== b.locationType) return false;
      return a.ruleKey === b.ruleKey || a.title.toLowerCase() === b.title.toLowerCase();
    }
  ...
  private static mergeIssues(target: CritiqIssue, source: CritiqIssue): void {
    ...
    // Combine locationConfidence (take the highest confidence)
    const targetLocConf = target.locationConfidence !== undefined ? target.locationConfidence : 85;
    const sourceLocConf = source.locationConfidence !== undefined ? source.locationConfidence : 85;
    target.locationConfidence = Math.max(targetLocConf, sourceLocConf);
    ...
    // 5. Union bounding boxes to cover both visual points (only if element-specific)
    if (target.locationType !== 'global' && target.boundingBox && source.boundingBox) {
   ...
   */
  private static calculateOverlap(boxA: BoundingBox | null, boxB: BoundingBox | null): number {
   ...
   */
  private static calculateDistance(boxA: BoundingBox | null, boxB: BoundingBox | null): number {
    if (!boxA || !boxB) return 999;
