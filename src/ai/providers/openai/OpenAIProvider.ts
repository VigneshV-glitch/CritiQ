  ...
  async validateConnection(): Promise<{ isValid: boolean; latencyMs: number; message: string; model: string }> {
    ...
    try {
      ...
      return {
        ...
        message: data.message || 'OpenAI API connection active.',
      ...
      return {
        isValid: false,
        latencyMs: Date.now() - start,
        message: e.message || 'Failed connecting to OpenAI.',
  ...
  async analyzeDesign(request: AnalyzeDesignRequest): Promise<NormalizedAIResponse> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw normalizeAIError(errJson.message || `HTTP ${response.status}`, 'openai', model);
      }

      const data = await response.json();
      const latencyMs = Date.now() - startTime;
      return normalizeRawAIResponse(data, 'openai', model, request.imageSrc, request.fileName, latencyMs);
    } catch (err: any) {
      throw normalizeAIError(err, 'openai', model);
    }
  }
