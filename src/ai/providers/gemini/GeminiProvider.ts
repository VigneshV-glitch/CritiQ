  ...
  async validateConnection(): Promise<{ isValid: boolean; latencyMs: number; message: string; model: string }> {
    ...
    try {
      ...
      return {
        ...
        message: data.message || 'Gemini API connection active.',
      ...
      return {
        isValid: false,
        latencyMs: Date.now() - start,
        message: e.message || 'Failed connecting to Gemini.',
  ...
  async analyzeDesign(request: AnalyzeDesignRequest): Promise<NormalizedAIResponse> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw normalizeAIError(errJson.message || `HTTP ${response.status}`, 'gemini', model);
      }

      const data = await response.json();
      const latencyMs = Date.now() - startTime;
      return normalizeRawAIResponse(data, 'gemini', model, request.imageSrc, request.fileName, latencyMs);
    } catch (err: any) {
      throw normalizeAIError(err, 'gemini', model);
    }
  }
