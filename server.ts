...
import { IntentInterpreter } from './src/lib/critiq-engine/intentInterpreter';
...
function getSimulatedAnalysis(
  ...
  for (let i = 0; i < numIssues; i++) {
    ...
    const isGlobal = poolItem.ruleKey === 'hicks_law';
    
    // For demoing low location confidence
    let locConfidence = 70 + ((hash + i) % 25);
    if (poolItem.title.toLowerCase().includes('rounding') || poolItem.title.toLowerCase().includes('deviation') || poolItem.title.toLowerCase().includes('spacing')) {
      locConfidence = 45;
    }
    ...
    demoIssues.push({
      ...
      locationConfidence: isGlobal ? 100 : locConfidence,
      recommendation: rec,
...
function validateQualityAssurance(parsedData: any, vu: any): { isValid: boolean; reason?: string } {
  ...
  for (const issue of parsedData.issues) {
    ...
    const isGlobal = issue.locationType === 'global' || !bbox || (bbox.x === 0 && bbox.y === 0 && bbox.width >= 98 && bbox.height >= 98);
...
- Allow any clean, legible typography and look-and-feel as long as visual hierarchy is distinct and readable.`;
    ...
    const finalReport = await runWithStrategicRetries<any>(async (attempt, extraWarning) => {
      ...
      const backwardIssues = multiAgentReport.prioritizedIssues.map((iss, idx) => {
        ...
        const locationType = iss.locationType || (box.width >= 95 && box.height >= 95 ? 'global' : 'element');
        const locationConfidence = iss.locationConfidence || (locationType === 'global' ? 100 : 85);
        ...
        return {
          ...
          boundingBox: locationType === 'global' ? null : box,
          locationType,
          locationConfidence,
